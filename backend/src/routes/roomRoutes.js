...
router.post(
  ...
  requireRole(["admin"]),
  [
    body("hotelName").isString().trim().notEmpty(),
    body("location").isString().trim().notEmpty(),
    body("basePricePerNight").isNumeric().toFloat(),
    body("maxGuests").isNumeric().toInt(),
  ],
  validateRequest(),
  createRoom
);
...
router.put(
  ...
  authMiddleware,
  requireRole(["admin"]),
  [param("id").isMongoId(), body("hotelName").optional().isString().trim().notEmpty()],
  validateRequest(),
  updateRoom
);
...
// Pricing calculator for booking UI (public)
